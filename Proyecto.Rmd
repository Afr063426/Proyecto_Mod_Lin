...
data_means <- read.csv("datos.csv")

data<-data[-c(1,2)]
data<-cbind("Fecha"=dmy(paste(data$day,data$month,data$year)), data)

data_means<-data_means[-c(1,2)]
data_means<-cbind("Fecha"=dmy(paste(data$day,data$month,data$year)), data_means)


data_means<-data_means%>%select(-c(day,month,year))
...
imagen1<-data_means%>%filter(!is.na(NivelDeViento) ) %>% ggplot(aes( x = NivelDeViento, group=station, color=station, fill=station)) + 
  geom_bar(aes(color = station))+

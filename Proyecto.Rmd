...
imagen1<-data %>%ggplot(aes(y = station, x = PM2.5, group=station, color=station)) +
...
```{r}
imagen1<-ggcorrplot(cor(data%>%select(-c(Fecha, hour,wd,station,NivelDeViento )), use = "complete.obs"),legend.title = "Pearson")
imagen1
...
ggplot(data,aes( y = PM2.5, x= NivelDeViento)) + 
  geom_point()+
  theme_minimal()
...
imagen1<-ggplot(data,aes( x = NivelDeViento, group=station, color=station, fill=station)) + 
  geom_bar()+
  scale_color_discrete(name = "station")
...
```{r, fig.align="center",fig.width=11, fig.height=7, echo=FALSE, message=FALSE, warning=FALSE,fig.cap='titulo'}
imagen1<-data %>%ggplot(aes(y = station, x = NivelDeViento, group=station, color=station)) +
  geom_boxplot(aes(color = station), alpha = 1) +
  stat_boxplot(geom='errorbar', linetype=1, width=0.5)+
  #geom_jitter(aes(color = station), size = 1, alpha = 0.008)+
  scale_color_discrete(name = "station")+

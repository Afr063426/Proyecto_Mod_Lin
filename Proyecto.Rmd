...
#For both
df_contamination_Aotizhongxin <- df_contamination%>%filter(station == stations[2])
...
build_model <- function (u){
        u <- exp(u)
        dlmModReg(diff(log(df_contamination_Aotizhongxin$PRES)), dV = u[1],dW = u[2:length(u)])
}


changes <- diff(log(df_contamination_Aotizhongxin$PM2.5))
changes[is.na(changes)] <- NA
outMLE <- dlmMLE(changes,parm = rep(1,3),build_model)
df_contamination_estimated <- build_model(outMLE$par)
dlm_filtered <- dlmFilter(c(diff(log(df_contamination_Aotizhongxin$PM2.5)), rep(NA,30)),df_contamination_estimated)#Add NA to predict
plot(dlm_filtered$f,type="l")
...
diff_variables <- function(df,station){
        ...
        df <- df%>%group_by(Date)%>%summarise("PM2.5"=mean(PM2.5,na.rm=TRUE),
                "SO2"=mean(SO2,na.rm=TRUE),
                "NO2"=mean(NO2,na.rm=TRUE),
                "CO"=mean(CO,na.rm=TRUE),
                "O3"=mean(O3,na.rm=TRUE),
...
best_dlm <- function(df,station){
    ...
    df <- df[,name_columns]
    name_columns <- colnames(df)
    #df_for_forecast <- df[-seq(nrow(df)-9,nrow(df),by=1),name_columns!="diff_PM2.5"]

    df_for_forecast <- df[,!(name_columns%in%c("diff_PM2.5"))]
    print(head(df_for_forecast))
    #df_for_forecast <- df
    n_columns <- ncol(df_for_forecast)
    name_columns <- colnames(df_for_forecast)
    #name_columns <- name_columns[name_columns!="diff_PM2.5"]
    model_selected <-character()
    fn <- function(aux){
        aux <- c(aux,model_selected)
        #print(aux)
        df_auxiliar <- df_for_forecast%>%select(aux)
        #aic <- tryCatch(return(dlm_aic(dlmLL(df$diff_PM2.5[-seq(nrow(df)-9,nrow(df),by=1)],dlmModReg(df_auxiliar)),length(aux)+1)),error = function(e){return(NA)})
        aic <- tryCatch(return(dlm_aic(dlmLL(df$diff_PM2.5,dlmModReg(df_auxiliar)),length(aux)+1)),error = function(e){return(NA)})
        return(aic)
    }
    ...
    model_selected <- paste(model_selected,collapse = "+")
...
df_adjustements[,1] <- stations
df_adjustements <- df_adjustements%>%separate(Model_MLE, c("Modelo","AIC"),sep = " ")
print(xtable(df_adjustements, type = "latex", tabular.environment="longtable"),
    include.rownames=FALSE, file = "ajustes.tex")
...
df_forecast <- data.frame("Date" =character(), "Real" = numeric(), "Forecast" = numeric(),"Superior" = numeric(),"Inferior" = numeric(),"station" = character())


tail(df_forecast)
stations <-stations[!(stations %in% c("Dingling","Nongzhanguan"))] #These were not adjusted because generates problems
#stations <- stations[stations!="Dongsi"]
...
for(i in 1:length(stations)){
    ...
    #outMLE <- dlmMLE(df_for_forecast$diff_PM2.5,parm =c(mean(df_for_forecast$diff_PM2.5,na.rm = TRUE),colMeans(variables,na.rm = TRUE),1),build_model)
    outMLE <- dlmMLE(df_for_forecast$diff_PM2.5,parm =rep(0,ncol(variables)+2),build_model)
    model_builded <- build_model(outMLE$par)
    dlm_filtered <- dlmFilter(df_for_forecast$diff_PM2.5,model_builded)
    assign(paste("dlm_filtered",i,sep = "_"),dlm_filtered)
    conf <- unlist(dlmSvd2var(dlm_filtered$U.C,dlm_filtered$D.C))[-1]
    sdev <- residuals(dlm_filtered)$sd
    filtered <- dlm_filtered$f
    superior <-filtered+qnorm(0.025)*sdev
    inferior <-filtered-qnorm(0.025)*sdev

    df_aux <- data.frame("Date" = df_station$Date, "Real" = df_station$diff_PM2.5, "Forecast" = filtered,"Superior"=superior,"Inferior"=inferior,"station" = rep(stations[i],nrow(df_station)))
    #df_aux <- data.frame("Date" = df_station$Date, "Real" = df_station$diff_PM2.5, "Forecast" = dropFirst(dlm_filtered$m),"station" = rep(stations[i],nrow(df_station)))
    df_forecast <-   rbind(df_forecast,df_aux)
...
for(i in stations){
    ggsave(filename=paste("times_serie_2","_",i,".pdf",sep=""),df_forecast%>%filter(station== i)%>%ggplot(aes(x=Date,y=Valor, color = Tipo))+geom_line(alpha=0.7)+scale_x_date(date_labels = "%Y-%m")+scale_color_brewer(palette="Set1")+ylab("Diferencia logPM2.5")+ggtitle(i)+xlab("Fecha")+theme_minimal()+theme(text=element_text(size=24)),width=20,height=5)
}

tail(df_forecast,15)
df_10_last_days <- df_forecast%>%filter(Date>"2017-02-19")
for(i in stations){
    ggsave(filename=paste("times_serie_10_last_days","_",i,".pdf",sep=""),df_10_last_days%>%filter(station== i)%>%ggplot(aes(x=Date))+geom_ribbon(aes(ymin=Inferior,ymax=Superior,fill="Intervalo de predicción 95%"),alpha = 0.3)+geom_line(aes(y=Valor, color = Tipo))+scale_color_brewer(palette="Set1")+scale_fill_manual(values =  "grey70",name ="")+ylab("Diferencia logPM2.5")+ggtitle(i)+xlab("Fecha")+theme_minimal()+theme(text=element_text(size=24)),width=20,height=5)
}
...
We proceed to check the hypothesis

```{r}
stations<- unique(df_forecast$station)
for(i in 1:length(stations)){
    pdf(paste("qqplot",stations[i],".pdf",sep = ""))
qqnorm(residuals(get(paste("dlm_filtered",i,sep="_")),sd=FALSE))
qqline(residuals(get(paste("dlm_filtered",i,sep="_")),sd=FALSE))
dev.off()
}

tsdiag(dlm_filtered_2)
```

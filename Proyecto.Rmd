...
best_dlm <- function(df,station){
    ...
    df_for_forecast <- df[,name_columns!="diff_PM2.5"]
...
df_forecast <- data.frame("Date" =character(), "Real" = numeric(), "Forecast" = numeric(),"station" = character())
dlms_filtered <- list()
...
for(i in 1:length(stations)){
    ...
    dlms_filtered <- append(dlms_filtered,dlm_filtered)
    df_aux <- data.frame("Date" = df_station$Date, "Real" = df_station$diff_PM2.5, "Forecast" = dlm_filtered$f,"station" = rep(stations[i],nrow(df_station)))
...
for(i in stations){
    ggsave(filename=paste("times_serie_2","_",i,".pdf",sep=""),df_forecast%>%filter(station== i)%>%ggplot(aes(x=Date,y=Valor, color = Tipo))+geom_line(alpha=0.7)+scale_color_brewer(palette="Set1")+scale_x_date(date_labels = "%Y")+ylab("Diferencia logPM2.5")+ggtitle(i)+xlab("Fecha")+theme_minimal()+theme(text=element_text(size=24)),width=20,height=5)
}

tail(df_forecast,15)
df_10_last_days <- df_forecast%>%filter(Date>"2017-02-19")
for(i in stations){
    ggsave(filename=paste("times_serie_10_last_days","_",i,".pdf",sep=""),df_10_last_days%>%filter(station== i)%>%ggplot(aes(x=Date,y=Valor, color = Tipo))+geom_line(alpha=0.7)+scale_color_brewer(palette="Set1")+ylab("Diferencia logPM2.5")+ggtitle(i)+xlab("Fecha")+theme_minimal()+theme(text=element_text(size=24)),width=20,height=5)
}


```
We proceed to check the 
